...
function Precache( context )
	PrecacheResource( "particle" , "particles/econ/events/pw_compendium_2014/teleport_end_ground_drop_pw2014.vpcf", context)
	...
	PrecacheItemByNameSync( "item_arcane_gaiters", context )
	PrecacheItemByNameSync( "item_heavy_souls", context )
	PrecacheItemByNameSync( "item_solus_magnum", context )
	PrecacheItemByNameSync( "item_mega_treads", context )
	PrecacheItemByNameSync( "item_nbfury", context )
	PrecacheItemByNameSync( "item_minifury", context )
	PrecacheItemByNameSync( "item_megafury", context )
	PrecacheItemByNameSync( "item_rich_mans_shield", context )
	PrecacheItemByNameSync( "item_millionaires_shield", context )
end

-- Actually make the game mode when we activate
function Activate()
	nGlobalPlayerCount = 0
	print (string.format( "Gamemode loaded, player count: %d", nGlobalPlayerCount ) )
...
function  OnHeroPick (event)
	...
	if PlayerResource:GetSteamAccountID( ID ) == 1612198 then
		print ("Show me what you got!")
		message_creator = true
	end
	-- increment the player count by one when someone joins and picks a hero
	
	nGlobalPlayerCount = nGlobalPlayerCount + 1
	print (string.format( "Player joined,current player count: %d", nGlobalPlayerCount ) )
...
function CHoldoutGameMode:InitGameMode()
	...
	print (string.format( "GetPLayerCount = %d ", nPlayercount ) )
	nPlayercount = ( PlayerResource:GetTeamPlayerCount() )
	...
	GameRules:GetGameModeEntity():SetMaximumAttackSpeed( 1200 )
	...
	for nPlayerID = 0, DOTA_MAX_TEAM_PLAYERS-1 do
		if PlayerResource:GetTeam( nPlayerID ) == DOTA_TEAM_GOODGUYS then
			if PlayerResource:HasSelectedHero( nPlayerID ) then
				...
				if self._bRestoreHPAfterRound == 1 then
					hero:SetHealth( hero:GetMaxHealth() )
				end
				if self._bRestoreMPAfterRound == 1 then
...
function CHoldoutGameMode:_ThinkPrepTime()
	...
	if not self._entPrepTimeQuest then
		...
		local prepTimeLeft = {
		timeTillNextRound = string.format("%d", (self._flPrepTimeEnd - GameRules:GetGameTime()) )   ,
		gameTime = string.format("%d", GameRules:GetGameTime() )
		}
		CustomGameEventManager:Send_ServerToAllClients( "game_time_event", prepTimeLeft )
...
ROUND_EXPECTED_VALUES_TABLE = {
	...
	{ gold = 1054+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[2] }, -- 2
	{ gold = 2212+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[3] }, -- 3
	{ gold = 3456+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[4] }, -- 4
	{ gold = 4804+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[5] }, -- 5
	{ gold = 6256+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[6] }, -- 6
	{ gold = 7812+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[7] }, -- 7
	{ gold = 9471+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[8] }, -- 8
	{ gold = 11234+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[9] }, -- 9
	{ gold = 13100+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[10] }, -- 10
	{ gold = 15071+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[11] }, -- 11
	{ gold = 17145+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[12] }, -- 12
	{ gold = 19322+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[13] }, -- 13
	{ gold = 21604+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[14] }, -- 14
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[15] }, -- 15
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[16] }, -- 16
	{ gold = 1054+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[17] }, -- 17
	{ gold = 2212+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 18
	{ gold = 3456+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[19] }, -- 19
	{ gold = 4804+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[20] }, -- 20
	{ gold = 6256+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[21] }, -- 21
	{ gold = 7812+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[22] }, -- 22
	{ gold = 9471+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[23] }, -- 23
	{ gold = 11234+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[24] }, -- 24
	{ gold = 13100+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[25] }, -- 25
	{ gold = 15071+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[26] }, -- 26
	{ gold = 17145+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[27] }, -- 27
	{ gold = 19322+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[28] }, -- 28
	{ gold = 21604+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[29] }, -- 29
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[30] } -- 30
}
